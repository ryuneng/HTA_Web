...
public class DownloadServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			FileBoard fb = fileBoardDao.getFileBoardsByNo(fileBoardNo);
			...
			// 응답메시지의 응답헤더에 다운로드되는 첨부파일의 이름을 지정한다.
			// Content-Disposition : 응답 메시지의 바디부에 오는 컨텐츠의 특성을 알려주는 헤더정보
			...
			// InputStream로 읽어온 데이터를 OutputStream으로 내려보낸다.
			IOUtils.copy(in, out);
