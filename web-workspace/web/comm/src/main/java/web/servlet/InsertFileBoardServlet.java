...
/*
 * @WebServlet
 * 		- 이 클래스가 HTTP 요청을 처리하는 서블릿으로 동작하게 하는 어노테이션
 * 		- urlPatterns : 이 서블릿을 실행하기 위한 요청 URL을 지정
 * 
 * @MultipartConfig
 * 		- 멀티파트 요청을 처리할 수 있는 서블릿으로 동작하게 하는 어노테이션
 * 		- 이 어노테이션이 지정되어 있지 않으면 요청파라미터값을 조회할 수 없고, 첨부파일 업로드도 불가
 * 
 */
...
public class InsertFileBoardServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			// 새 파일게시글 정보를 insertFileBoard(FileBoard fileBoard)
			// 메소드 실행 시 전달해서 저장시킨다.
