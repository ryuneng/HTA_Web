	...
	/*
		...
		요청 파라미터
			...
			* page는 조회했을 때 null이거나 숫자가 조회된다.
				* null일 때 : 내비바의 메뉴를 클릭했을 때
					comm/board/list.jsp
			* opt와 keyword는 조회했을 때 null, 빈문자열, 문자열이 조회된다.
				* null일 때 : 내비바의 메뉴를 클릭했을 때
					comm/board/list.jsp
				* 빈 문자열일 때 : 게시글목록페이지에서 검색없이 단순히 페이지번호만 클릭했을 때
					comm/board/list.jsp?page=2&opt=&keyword=
				* 값이 있을 때 : 옵션과 검색어를 입력하고 검색버튼을 클릭했을 때
					comm/board/list.jsp?page=1&opt=title&keyword=연습
				* 값이 있을 때 : 검색된 목록페이지에서 페이지번호를 클릭했을 때
...
%>
				<li class="page-item">
					<a class="page-link"
					   ...
					   onclick="goPage(<%=currentPage - 1 %>, event)"> <!-- a태그 : 기본동작 막기 위해 event객체도 같이 전달 -->
					   이전
					</a>
				</li>
<%
...
%>
		...
		<div>
			<form id="form-search" class="row row-cols-sm-auto g-3 align-items-center"
				...
				<div class="col-12">
					<select class="form-select" name="opt"> <!-- boards.xml에 isEqual로 한 세트로 묶여있기 때문에 이벤트는 버튼에만 주면 됨 -->
...
<script type="text/javascript"> /* 20240119 */
	function changeRows() {
	...
	// 페이지 번호를 클릭했을 때 실행되는 이벤트핸들러 함수
	// 1. 검색폼의 page hidden 필드의 값을 전달받은 페이지값으로 설정한다.
	// 2. 검색폼의 폼입력값을 서버로 제출시킨다.
	// * 페이지번호를 클릭했을 때 검색폼의 검색조건과 검색어도 함께 서버로 제출해야 한다.
	//   현재 보고 있는 페이지가 검색결과를 표시하는 페이지라면 검색조건, 검색어와 클릭한 페이지번호도 전달되어야 한다.
	// * 따라서, 페이지번호를 클릭했을 때도 검색폼을 제출해야 한다.
	function goPage(page, event) { <!-- 기본동작 막기 위해 event객체도 같이 전달 -->
	...
	// 검색버튼을 클릭했을 때 실행되는 이벤트핸들러 함수
	// 1. 검색폼의 page hidden 필드의 값을 1로 설정한다.
	// 2. 검색폼의 폼입력값을 서버로 제출시킨다.
	// * 검색버튼을 클릭했을 때는 언제나 검색결과에 대한 1페이지를 요청하는 것이기 때문에 페이지번호를 1로 설정한다.
